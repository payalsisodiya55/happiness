...
const fixPricingViaAPI = async (adminToken) => {
  try {
    console.log('🚀 Starting to fix pricing via API...');
    ...
    console.error('❌ Error fixing pricing:', error.response?.data?.message || error.message);
  }
};
...
// Main function
const main = async () => {
  console.log('🚀 Starting pricing fix script...');
  
  const adminToken = getAdminToken();
  await fixPricingViaAPI(adminToken);
  
  console.log('\n✅ Script completed!');
  console.log('\n🎯 Now check your admin panel - you should see all 31 vehicles with pricing!');
};
...
module.exports = { generateAllPricingData, fixPricingViaAPI };
