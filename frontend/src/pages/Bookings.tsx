...
const sampleBookings = [
  ...
  },
  {
    ...
    payment: { status: "completed", method: "phonepe", transactionId: "pay_123456" }
  },
  {
    _id: "sample_3",
      ...
      return (
        <div className="space-y-0.5">
          ...
          <div className="flex items-center space-x-1.5">
            <div className={`w-1.5 h-1.5 rounded-full ${onlinePaymentStatus === 'completed' ? 'bg-green-500' : 'bg-yellow-500'
              }`}></div>
          ...
          <div className="flex items-center space-x-1.5">
            <div className={`w-1.5 h-1.5 rounded-full ${cashPaymentStatus === 'collected' ? 'bg-green-500' : 'bg-yellow-500'
              }`}></div>
            <span className="text-xs text-gray-700">
    ...
    return (
      <div className="space-y-0.5">
        ...
        <div className="text-xs text-gray-700 capitalize">
          {payment.method === 'phonepe' ? 'Online Payment' :
            payment.method === 'cash' ? 'Cash Payment' :
              payment.method || 'Unknown'}
        </div>
        <div className="text-xs text-gray-600">Payment Status:</div>
        <div className="flex items-center space-x-1.5">
          <div className={`w-1.5 h-1.5 rounded-full ${payment.status === 'completed' ? 'bg-green-500' : 'bg-yellow-500'
            }`}></div>
          <span className="text-xs text-gray-700">
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="sticky top-0 z-40 flex border-b border-gray-200 bg-white flex-shrink-0 shadow-sm">
        <button
          className={`flex-1 py-4 text-sm font-semibold transition-colors ${activeTab === "upcoming"
        ...
        <button
          className={`flex-1 py-4 text-sm font-semibold transition-colors ${activeTab === "past"
            ? "text-[#f48432] border-b-2 border-[#f48432]"
            : "text-gray-500 hover:text-gray-700"
            }`}
              ...
              <div key={booking._id} className="bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all duration-300 overflow-hidden group">
                ...
                <div className="p-5 border-b border-gray-50 bg-gray-50/30">
                  <div className="flex justify-between items-start gap-3">
                    ...
                    <div className={`px-3 py-1 text-xs font-semibold rounded-full capitalize shadow-sm border ${['accepted', 'started'].includes(booking.status)
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div className="flex items-center justify-between bg-gray-50/50 rounded-lg p-2 px-3 border border-gray-100">
                    <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full ${booking.payment?.status === 'completed' ? 'bg-green-500' : 'bg-yellow-500'
                        }`} />
                      <span className="text-xs font-medium text-gray-600">
                        Payment: {booking.payment?.method === 'phonepe' ? 'Online' : 'Cash'}
                      </span>
                    </div>
                    <span className={`text-xs font-semibold ${booking.payment?.status === 'completed' ? 'text-green-600' : 'text-yellow-600'
                      }`}>
              ...
              <>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <div className="flex justify-between items-start mb-6">
                      ...
                      <div className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide border ${['accepted', 'started'].includes(selectedBooking.status) ? "bg-green-50 text-green-700 border-green-100" :
                ...
                <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm space-y-4">
                  ...
                  <div className="bg-gray-50 rounded-lg p-3 text-xs flex justify-between items-center border border-gray-100">
                    ...
                    <div className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-wide ${selectedBooking.payment?.status === 'completed' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                      }`}>
